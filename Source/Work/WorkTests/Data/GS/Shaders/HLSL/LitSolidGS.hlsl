...
[maxvertexcount(4)]
void GS(triangle GSPS_INPUT input[3], inout TriangleStream<GSPS_INPUT> OutputStream)
{
    float3 edge1 = input[1].oWorldPos.xyz - input[0].oWorldPos.xyz;
    float3 edge2 = input[2].oWorldPos.xyz - input[0].oWorldPos.xyz;
    float3 faceNormal = normalize(cross(edge1,edge2));
    //float3 faceNormal = normalize(input[0].oNormal+input[1].oNormal+input[2].oNormal);
	for(int i = 0; i < 3; i++)
	{
        GSPS_INPUT outs = input[i];
        //outs.oWorldPos.xyz += faceNormal * 0.02;
        //outs.oPos = GetClipPos(outs.oWorldPos);
    	OutputStream.Append(outs);
    }
    GSPS_INPUT outs = input[2];
    outs.oWorldPos.xyz += normalize(edge1) * 0.5;
    outs.oPos = GetClipPos(outs.oWorldPos);
    outs.oTexCoord.xy = float2(0.5,0.5);
    OutputStream.Append(outs);
}
#endif
